...
impl ClientRequest {
    ...
    fn serialize_put(
        ...
        let pem = match public_key {
            ...
            Some(public_key) => match public_key.to_public_key_pem() {
                Err(e) => {
                    return Err(DeonError::RsaPkcs8Error(format!(
                ...
                Ok(pem) => {
                    ...
                    if !payload.is_empty() {
                        payload = match Self::encrypt(&public_key, payload) {
                            Err(e) => {
                                return Err(DeonError::RsaCryptoError(format!(
    ...
    fn serialize_pop(
        ...
        let secret_key = match secret_key {
            None => {
                return Err(DeonError::MissingSkError(format!(
                    "Cannot sign pop request without secret key"
                )))
            }
    ...
    fn deserialize_pop(b: &[u8]) -> Result<Self, DeonError> {
        ...
        match Self::verify(&public_key, &digest_in, sig) {
            Err(e) => Err(DeonError::RsaCryptoError(format!(
